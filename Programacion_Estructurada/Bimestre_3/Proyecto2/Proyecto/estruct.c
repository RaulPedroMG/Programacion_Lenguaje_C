/* Nombre: estruct.c
  Descripción: Contiene la definición de las funciones utilizadas en el programa.
  Autor: Equipo 1.
*/
...
void LeerArchContra(char *contra2)
{
	FILE *arch;
	 if ((arch = fopen("Contraseña.rpm", "r"))	 == NULL)
	 {
	...
	else
	{
		while (!feof(arch))
		{
			fgets(contra2,30,arch);
		}
	}
	//mvprintw(3,50,"La contraseña es %s ",contra2);
	fclose(arch);
}

void LeerArchivo(Empresa *catemp,int *cont)
{
	FILE *arch;
	char cad[450];
	initscr();
...
void GuardarEmpText(Empresa *catemp,int cont)
{
	...
	FILE *arch;
	printf("\nAbriendo el archivo...\n");
	if ((arch = fopen("./empresas3.txt", "w"))	 == NULL)
	{
		fprintf(stderr, "No se puede abrir el archivo de salida.\n");
		return;
	}
	...
	for (i=0; i < cont; i++)
		fprintf(arch,"%s\t%s\t%s\t%s\t%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n",catemp[i].clave,catemp[i].nombremp,catemp[i].giro,catemp[i].domicilio,catemp[i].estado,catemp[i].cp,catemp[i].telefono,catemp[i].fax,catemp[i].nomcont,catemp[i].correo,catemp[i].visita,catemp[i].estancia,catemp[i].servicios);
	fclose(arch);
}
...
/*
void LeerArchivoVisita(Visita *lisvisita,int *cont)
{
	FILE *arch;
	char cad[450];
	initscr();
	keypad(stdscr, TRUE);
	 if ((arch = fopen("./visitas.txt", "r"))	 == NULL)
	 {
			fprintf(stderr,"\t\tNo se puede abrir el archivo de salida.\n");
	 }
	else
	{
		while (!feof(arch))
		{
			if (fgets(cad,449,arch) != NULL)
			{
				SeparaCadVisita(lisvisita, cont, cad);
...
void SeparaCadVisita(Visita *lisvisita, int *cont, char *cad)
{
	int campo,i,j;
	char subc[200];
	for (campo=0,i=0, j=0; i < strlen(cad);i++)
...
void Empresas (Empresa *catemp,int *cont)
{
	...
	do
	{
		...
		if(opcion>=1 && opcion <=4)
		{
			switch (opcion)
			{
				case 1: //RegistrarEmp(catemp, cont);
					break;
				
				case 2: EliminarEmp(catemp, cont);
					
					break;
		
				case 3:	ModificarEmp(catemp, *cont);
					break;
						...
						switch (opcion)
						{
							...
							case 5 : clear(); 
									...
									else
									{
										...
										{
											mvprintw(24,100," Pulse enter para salir");
											Error(" La matrícula es inválida ! Verifícala",18);
										}
									}
								break;
...
void ImprimirEmp2 (Empresa *catemp, int t)
{
	...
	for (ind = 0; ind < t; ind++)
	{
		...
		mvprintw(5,2,"Clave: %s",catemp[ind].clave);
...
void ImprimirEmp (Empresa *catemp, int i)
{
	Centrar(catemp[i].nombremp,2);
	mvprintw(4,2,"Registro: %d",i+1);
	mvprintw(5,2,"Clave: %s",catemp[i].clave);
...
void Alumnos (Alumno *catalumn,int *cont)
{
	...
	do
	{
		...
		if(opcion>=1 && opcion <=4)
		{
			switch (opcion)
			{
				case 1: RegistrarAlumn(catalumn, cont);
						
					break;
				
				case 2: EliminarAlumn(catalumn, cont);
				
					break;
		
				case 3:ModificarAlumn(catalumn, *cont);
					break;
...
void RegistarVisita(Visita *lisvisita,int *t,Profesor *catprofe, int *to, Empresa *catemp, int *cont)
{
	...
	do
	{
		...
		Centrar("I N S E R T A R   A L U M N O ",2);
		mvprintw(4, 100, " Rellene los datos ");
		mvprintw(7, 2, " Clave Profe: ");
		LeerClav(cad, 7, 15);
		if (cad[0] != '\0')
		{
			ind = BuscarProfe(catprofe, *to, cad);
			if (ind != -1)
			{
				mvprintw(24,100," Pulse enter para salir");
				strcpy(lisvisita[*t].claveprofe, cad);
				mvprintw(8, 2, " Nombre :%s ",catprofe[ind].nomprofe);
				mvprintw(9, 2, " Apellidos :%s ",catprofe[ind].apeprofe);
				mvprintw(10, 2, " Clave Empresa: ");
				LeerClave(cad,10,17);
				if(cad[0] != '\0')
				{
					i=BuscarEmp(catemp, *cont, cad);
					if(i!=-1)
					{
						strcpy(lisvisita[*t].clavemp, cad);
						mvprintw(11, 2, " Nombre :%s ",catemp[i].nombremp);
						mvprintw(12, 2, " Fecha : ");
						LeeFecha(cad,12,10);
						if(cad[0] != '\0')
						{
							strcpy(lisvisita[*t].fechavis, cad);
							mvprintw(13, 2, " Calificación: ");
							res=LeeCalif(num);
							if(res>=1 || res<=5)
							{
								switch(res)
								{
									case 1:
										LeerMal(ca);
										lisvisita[*t].calif = (char *) malloc(strlen(ca));
										strcpy(lisvisita[*t].calif, ca);
										break;
									case 2:
										LeerRegu(ca);
										lisvisita[*t].calif = (char *) malloc(strlen(ca));
					...
					{
						Error("la clave no existe verificala por favor!",18);
					}	
				}
				else
				{
				...
				do
				{
					...
					if (opcion >= 1 && opcion <= 3)
					{
						switch (opcion)
						{
							...
							case 3 : clear(); 
									Centrar(" MODIFICAR APELLIDO DEL ALUMNO ",2);
									mvprintw(10,4," Escriba el nuevo apellido del alumno : ");
									mvprintw(14,25," Pulse enter para cancelar");
									LeerApellidoAlumn(cad,10,45);
									if (cad[0] != '\0')
									{
										catalumn[ind].apealum = (char *) malloc(strlen(cad));
										strcpy(catalumn[ind].apealum, cad);
