...
void ingresardatos()
{
	...
	switch(opc){
		...
		case 2:
			clear();
			attron(COLOR_PAIR(7));
			mvprintw(2,40,"IMPRESION DE LOS DATOS DE AVIONES");
			attron(COLOR_PAIR(3));
			mvprintw(5,4,"	CLAVE		DESTINO			CAPITAN DE VUELO		AEROLINEA");
			attron(COLOR_PAIR(2));
			for(i=0;i<V;i++){
				echo();
		...
		if (cad[i] != '\t' && cad[i] != '\n')
			...
			switch (campo){
				case 1 : lista[*total].clave_v = atoi(subc);
						break;
				case 2 : lista[*total].destino = (char *) malloc(strlen(subc)+1);
						strcpy(lista[*total].destino,subc);
						break;
				case 3 : lista[*total].cap_vuelo = (char *) malloc(strlen(subc)+1);
						strcpy(lista[*total].cap_vuelo,subc);
						break;
				case 4 : lista[*total].aerolinea = (char *) malloc(strlen(subc)+1);
...
void LlenaCola(Cola *pis1, Cola *pis2, Cola *pis3)
{
	...
	for (i=1; i<=15;i++){
		...
		clear();
		attron(COLOR_PAIR(3));
		mvprintw(15,70,"Cargando aviones");
...
void AgregarVuelo(Cola *aux1, Cola *aux2, Cola *aux3,  Vuelos *turno)
{
	...
	do{
		...
		r = cont % 3;
		attron(COLOR_PAIR(3));
		mvprintw(15,70,"Asignando turno");
		attroff(COLOR_PAIR(3));
		if(r==0){
			attron(COLOR_PAIR(10));
			marco(18,73,2,1,' ',' ');
...
void Findes (int nua)
{
	clear();
	attron(COLOR_PAIR(10));
	MarcoenRPM(15,45,40,10,'_','|',' ');
	mvprintw(16,55,"Cerrando aeropuerto");
	attroff(COLOR_PAIR(10));
	attron(COLOR_PAIR(13));
	mvprintw(19,48," Total de aviones despagados :  %d ", nua);
...
void MarcoenRPM(int filaO, int columnaO, int largo, int ancho, char borde_a, char borde_l, char relleno)
{
	int i,j;
	...
	for(i=filaO; i<filaO+ancho;i++)	
		for(j=columnaO; j<columnaO+largo;j++){
			if(i==filaO || i==filaO+ancho-1){
				mvprintw(i+3,j,"%c",borde_a);
			}
			else{ 
				if(j==columnaO || j==columnaO+largo-1){
					mvprintw(i,j,"%c",borde_l);
				}
				else{ 
					mvprintw(i,j,"%c",relleno);
				}
...
int MenuAd()
{
	...
	attron(COLOR_PAIR(10));
	marco(5,36,78,30,' ',' ');
	mvprintw(6,67," Opciones a elegir");
	mvprintw(14,50,"1.- Simulacion de aviones");
	mvprintw(16,50,"2.- Ver lista de aviones");
	mvprintw(18,50,"3.- Salir");
	mvprintw(22,50,"ELIJE UNA OPCION");
	attroff(COLOR_PAIR(10));
	attron(COLOR_PAIR(3));
	j=Rango(1,3,22,73);
	attroff(COLOR_PAIR(3));
