...
void ventana_acerca_de(){
    dialog_info = gtk_message_dialog_new(GTK_WINDOW(window), 0, GTK_MESSAGE_INFO,
                    GTK_BUTTONS_NONE, "RECONOCEDOR DE PALABRAS\n\nDesarrolladores : \n\nDaniel Cruz Santos\ncanuardaniel@hotmail.com\n\nIsabel Jiménez Canseco\nmariaisabella_28061993@hotmail.com\n\nDeysi Janet Cruz Ruiz\nisyedtenaj@hotmail.com\n\nRaúl Pedro Méndez García\nraulpedromega@gmail.com");
        gtk_dialog_add_buttons(GTK_DIALOG(dialog_info), GTK_STOCK_OK, GTK_RESPONSE_OK, NULL, NULL, NULL, NULL);
        gtk_dialog_run(GTK_DIALOG(dialog_info));
        gtk_widget_destroy(dialog_info);
}

void ventana_ayuda(){
    dialog_info = gtk_message_dialog_new(GTK_WINDOW(window), 0, GTK_MESSAGE_QUESTION,
                    GTK_BUTTONS_NONE, "Ejemplo de escritura de un Autómata \n\n #Ejemplo de Automata\n #Alfabeto de entrada \n a,b\n #Conjunto de estados\n p,q,r,s,t,u,v\n #Estado inicial\n p\n #Estado de aceptacion\n u,v\n #Funciones de transicion\n q,r\n s,t\n u,v\n s,t\n u,v\n s,t\n u,v");
        gtk_dialog_add_buttons(GTK_DIALOG(dialog_info), GTK_STOCK_OK, GTK_RESPONSE_OK, NULL, NULL, NULL, NULL);
        gtk_dialog_run(GTK_DIALOG(dialog_info));
        gtk_widget_destroy(dialog_info);
}
...
void mensaje_error(char *mensaje){
    dialog_info = gtk_message_dialog_new(GTK_WINDOW(window), 0, GTK_MESSAGE_ERROR,
                    GTK_BUTTONS_NONE,
                    mensaje);
        gtk_dialog_add_buttons(GTK_DIALOG(dialog_info), GTK_STOCK_OK, GTK_RESPONSE_OK, NULL, NULL, NULL, NULL);
        gtk_dialog_run(GTK_DIALOG(dialog_info));
...
void mensaje_info_ven2(char *mensaje){
    dialog_info = gtk_message_dialog_new(GTK_WINDOW(window2), 0, GTK_MESSAGE_INFO,
                    GTK_BUTTONS_NONE,
                    mensaje);
        gtk_dialog_add_buttons(GTK_DIALOG(dialog_info), GTK_STOCK_OK, GTK_RESPONSE_OK, NULL, NULL, NULL, NULL);
        gtk_dialog_run(GTK_DIALOG(dialog_info));
...
void mensaje_error_ven2(char *mensaje){
    dialog_info = gtk_message_dialog_new(GTK_WINDOW(window2), 0, GTK_MESSAGE_ERROR,
                    GTK_BUTTONS_NONE,
                    mensaje);
        gtk_dialog_add_buttons(GTK_DIALOG(dialog_info), GTK_STOCK_OK, GTK_RESPONSE_OK, NULL, NULL, NULL, NULL);
        gtk_dialog_run(GTK_DIALOG(dialog_info));
...
/*void guardar_imagen(){
    ...
    GtkWidget *dialog = gtk_file_selection_new("Guardar Imagen...");
    int resp = gtk_dialog_run(GTK_DIALOG(dialog));
    if(resp == GTK_RESPONSE_OK) {
        filenameimage = g_strdup(
    ...
    if(resp == GTK_RESPONSE_OK){
        gtk_image_set_from_file (GTK_IMAGE(image2), filenameimage);
    }
}*/
...
static gboolean poner_image (){
    if(val_mat_trans == TRUE){
        ...
        //mensaje_error("Error en la creación del Autómata. \n      Verifique su escritura \n");
    }
}
...
static gboolean reconoce_cadena (){
    ...
    if(ind_text == (strlen(text)+2))
    {
        ...
        if(ind >= 0){
            mensaje_info_ven2("La cadena ES ACEPTADA");//printf("\nLa cadena es aceptada\n");
        }
        else{
            mensaje_info_ven2("La cadena NO ES ACEPTADA");
...
void boton_Reconocer(){
    ...
    if (validar_cadena() == TRUE){
        ...
        //printf("\n***La cadena es incorrecta***\n");
        mensaje_error_ven2("La cadena es incorrecta. Verifíquela.");
    }
...
void ventana_Reconocer(){
    if((gtk_text_buffer_get_modified(GTK_TEXT_BUFFER(buf)) ||  if_automata == TRUE))
    {
        if(val_mat_trans == TRUE)
        {
            ...
            g_signal_connect(G_OBJECT(window2), "destroy",G_CALLBACK(salir_ventana2), NULL);
    ...
    /*_______________________________________________________________________*/        
            ...
            mensaje_info("No existe un Autómata. Es necesario generarlo correctamente.");
        }
        
    }else{
            mensaje_info("Es necesario generar el Autómata.");
            //mensaje_info("");
...
void automata(){
    ...
    gtk_widget_add_accelerator(menuitem, "activate", accel, 'A', GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
    ...
    menuitem = gtk_menu_item_new_with_label(" Ayuda  ");
    gtk_menu_bar_append(GTK_MENU_BAR(menubar), menuitem);
    g_signal_connect_swapped (G_OBJECT (menuitem), "activate", G_CALLBACK (ventana_ayuda), NULL);
    ...
    gtk_menu_bar_append(GTK_MENU_BAR(menubar), menuitem);
    g_signal_connect_swapped (G_OBJECT (menuitem), "activate", G_CALLBACK (ventana_acerca_de), NULL);
...
//___________________________________________________________________________
    ...
    //g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (guardar_imagen), NULL);
...
/////////////////validando los valores de la matriz  .. sólo datos que estan en el vector de estados
        ...
        val_mat_trans = TRUE;
        //printf("\n Einicial : %d\n", Einicial);
        if (buscar_cadena(Einicial, Estados, fil) == -1){
            val_mat_trans = FALSE;
            mensaje_error("Se encontró un estado inicial incorrecto");
        }
        /*if(strlen(estado_ini) == 1){
            val_mat_trans = TRUE;
        }else{
            val_mat_trans = FALSE;
            mensaje_error("El estado inical debe ser un solo estado");
        }*/
        if(val_mat_trans == TRUE){
            for (i = 0; i < Eaceptacion; ++i)
            {
                if(buscar_cadena(aceptacion[i], Estados, fil) == -1){
                    val_mat_trans = FALSE;
                    mensaje_error("Se encontró un estado aceptor incorrecto");
                }
            }
        }
        if (val_mat_trans == TRUE)
        {
            ...
            /* finalmente, libera la zona de memoria */
                ...
                val_mat_trans = FALSE;
                mensaje_error("Error en la matríz de transición");
